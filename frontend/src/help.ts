...
export const HELP_TOPICS: Record<HelpTopicId, HelpTopic> = {
  overview: {
    ...
    summary:
      "Scholion turns local recordings into searchable, verifiable evidence without making the app database or search index the source of truth.",
    points: [
      "Add recordings or existing canonical transcript folders without moving your originals into a hidden vault.",
      "Process locally. Scholion inspects the recording and this computer before choosing an execution strategy.",
  ...
  intake: {
    ...
    summary:
      "This screen decides what local material Scholion may use. Selecting something does not silently copy, transcribe, or remove it.",
    points: [
      "Choose files for a one-time action, or choose a folder if you may want Scholion to remember that location.",
    ...
    note: "Forgetting a remembered folder removes Scholion's permission record. It does not change files inside the folder.",
  },
  processing: {
    title: "Processing recordings",
    summary:
      "Processing starts with a preflight. Python inspects source media, available hardware, memory, models, and requested outcome before Scholion admits a local job.",
    points: [
      "Choose an outcome profile rather than manually guessing thread counts or memory limits.",
      "Run preflight before starting so Scholion can verify the selected source, model, stream, and resource fit.",
  ...
  library: {
    ...
    points: [
      ...
      "Search rank is not evidence authority. Scholion verifies the canonical generation before precise navigation.",
    ],
  ...
  research: {
    ...
    points: [
      ...
      "If an older note cites an older transcript generation, Scholion can reopen that exact preserved evidence instead of silently moving the citation.",
  ...
  storage: {
    ...
    summary:
      "Storage changes are reviewable custody operations. Scholion calculates the exact consequences before it applies a cleanup or removal plan.",
    points: [
  ...
  playback: {
    ...
    summary:
      "Prepare playback first so Scholion can re-check the exact transcript generation and original recording before a native media session opens.",
    points: [
    ...
    note: "If verification succeeds but the operating system cannot decode the media codec, Scholion reports that separately from an evidence-integrity failure.",
  },
  "transcript-tools": {
