...
def verify_diarization_install(wheel: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="scholion-diarization-dist-") as temporary:
        ...
        requirement = f"scholion[diarization] @ {wheel.as_uri()}"
        env = os.environ.copy()
        ...
        _run(
            [
                ...
                "-c",
                (
                    "import scholion, pyannote.audio, torch; "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Verify that the built Scholion wheel's diarization extra installs and "
