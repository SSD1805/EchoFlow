...
from src.core.health_check import CheckStatus
...
def test_workspace_probe_reports_write_failure_without_leaking_exception(tmp_path):
    with patch(
        "src.core.health_probes.tempfile.NamedTemporaryFile",
...
def test_disk_threshold_boundaries(tmp_path, free, expected):
    ...
    with patch("src.core.health_probes.shutil.disk_usage", return_value=usage):
...
def test_disk_probe_walks_to_existing_parent_for_missing_workspace(tmp_path):
    ...
    with patch("src.core.health_probes.shutil.disk_usage") as disk_usage:
...
def test_ffmpeg_missing_is_optional_warning():
    with patch("src.core.health_probes.shutil.which", return_value=None):
...
def test_ffmpeg_is_invoked_without_a_shell():
    ...
    with (
        patch("src.core.health_probes.shutil.which", return_value="/usr/bin/ffmpeg"),
        patch("src.core.health_probes.subprocess.run", return_value=completed) as run,
    ):
...
def test_ffmpeg_timeout_is_warning():
    with (
        patch("src.core.health_probes.shutil.which", return_value="ffmpeg"),
        patch(
            "src.core.health_probes.subprocess.run",
...
def test_ffmpeg_nonzero_exit_is_warning():
    ...
    with (
        patch("src.core.health_probes.shutil.which", return_value="ffmpeg"),
        patch("src.core.health_probes.subprocess.run", return_value=completed),
    ):
        result = FfmpegProbe(0.1).check()
...
def test_system_resources_are_read_nonblockingly():
    ...
    with (
        patch("src.core.health_probes.psutil.cpu_count", return_value=4) as cpu,
        patch("src.core.health_probes.psutil.virtual_memory", return_value=memory),
    ):
...
def test_unknown_or_zero_resources_fail(cpus, available):
    ...
    with (
        patch("src.core.health_probes.psutil.cpu_count", return_value=cpus),
        patch("src.core.health_probes.psutil.virtual_memory", return_value=memory),
    ):
        result = SystemResourcesProbe().check()
