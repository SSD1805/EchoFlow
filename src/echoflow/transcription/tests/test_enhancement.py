...
from echoflow.transcription.enhancement import (
...
def test_enhancer_fails_closed_when_ffmpeg_is_unavailable(
    ...
    _write_wave(source)
    monkeypatch.setattr(
        "echoflow.transcription.enhancement.shutil.which", lambda _: None
    )
...
def test_enhancer_applies_fixed_filter_and_preserves_timeline(
    ...
    monkeypatch.setattr(
        "echoflow.transcription.enhancement.shutil.which", lambda _: "/usr/bin/ffmpeg"
    )
    ...
    monkeypatch.setattr("echoflow.transcription.enhancement.subprocess.run", fake_run)
