...
class WorkspaceService:
    ...
    def initialize(self, output_dir: str | Path | None = None) -> WorkspacePaths:
        ...
        for directory in (
            ...
            self.file_manager.ensure_directory_exists(directory)
        return layout
    ...
    def create_job(
        ...
        *,
        ...
        layout = self.initialize(output_dir)
        selected_id = job_id or JobId(self.id_factory())
        job_workspace = layout.jobs_dir / selected_id.value
        try:
            self.file_manager.reserve_directory(job_workspace)
        except StorageAlreadyExistsError as exc:
            raise JobCollisionError(selected_id.value, cause=exc) from exc
        return Job(
            job_id=selected_id,
            input_path=source,
    ...
    def _validate_job(self, job: Job) -> None:
        ...
        if job.workspace_dir != expected_workspace or not job.workspace_dir.is_dir():
