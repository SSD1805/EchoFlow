...
from scholion.cli_jobs import register_job_commands
from scholion.cli_library import register_library_commands
from scholion.cli_models import register_model_commands
from scholion.cli_progress import RichTranscriptionProgress
from scholion.core.config import AppConfig
from scholion.core.errors import ScholionError
from scholion.core.health_check import HealthReport
...
from scholion.transcription.export import TranscriptExportFormat, TranscriptExportResult
...
def root(
    ...
    config_file: Annotated[
        ...
        typer.Option(
            ...
            readable=True,
            resolve_path=True,
            help="Explicit Scholion dotenv configuration file.",
        ),
    ] = None,
) -> None:
    """Scholion command-line interface."""
...
def _render_report(report: HealthReport, console: Console) -> None:
    table = Table(title=f"Scholion doctor: {report.status.value}")
...
def _render_paths(paths: WorkspacePaths, console: Console) -> None:
    table = Table(title="Scholion directories initialized")
...
def _render_runner(
    ...
    table = Table(title="Scholion runner policy")
...
def _render_strategies(
    ...
    table = Table(title="Scholion local transcription strategies")
...
def _render_transcription_plan(plan: TranscriptionJobPlan, console: Console) -> None:
    ...
    table = Table(title="Scholion transcription dry run")
    table.add_column("Setting")
...
def _render_transcription_result(
    ...
    table = Table(title="Scholion transcription complete")
    table.add_column("Setting")
    table.add_column("Value")
    rows = [
...
def initialize(
    ...
    output_dir: Annotated[
        ...
        typer.Option(
            "--output-dir",
            file_okay=False,
            help="Directory for user-visible Scholion artifacts.",
        ),
    ] = None,
    ...
    try:
        ...
        typer.echo("Invalid Scholion configuration", err=True)
        raise typer.Exit(code=1) from None
    except Exception as exc:
        typer.echo(
            f"Scholion initialization failed internally ({type(exc).__name__})",
            err=True,
        )
...
def doctor(
    ...
    try:
        ...
        typer.echo(
            f"Scholion doctor failed internally ({type(exc).__name__})", err=True
        )
        raise typer.Exit(code=3) from None
...
def inspect_runner(
    ...
    try:
        ...
        typer.echo(
            f"Scholion runner inspection failed internally ({type(exc).__name__})",
            err=True,
        )
        raise typer.Exit(code=3) from None
...
def strategies(
    ...
    try:
        ...
        typer.echo(
            f"Scholion strategy inspection failed internally ({type(exc).__name__})",
            err=True,
        )
        raise typer.Exit(code=3) from None
...
def _execute_cli_plan(
    ...
    *,
    ...
    if resume is None:
        typer.echo(f"Scholion job ID: {plan.job.job_id.value}", err=True)
    if json_output:
...
def _report_transcription_interrupt(plan: TranscriptionJobPlan | None) -> None:
    typer.echo("", err=True)
    if plan is None:
        typer.echo("Scholion interrupted before the job started.", err=True)
        return
    typer.echo(
        "Scholion job interrupted. Your completed segments remain private and "
        "checkpointed.",
        err=True,
    )
    typer.echo(
        f"Resume: scholion transcribe {plan.job.input_path} "
...
def transcribe(
    ...
    strategy: Annotated[
        str | None,
        typer.Option(
            help="Explicit safe strategy ID from `scholion strategies`; never silently downgraded."
        ),
    ] = None,
    ...
    try:
        ...
        typer.echo(
            f"Scholion transcription failed internally ({type(exc).__name__})",
            err=True,
        )
        raise typer.Exit(code=3) from None
