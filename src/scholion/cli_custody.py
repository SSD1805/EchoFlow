...
from scholion.cli_refresh import register_refresh_command
...
def _render_deletion_plan(plan: DeletionPlan) -> None:
    table = Table(title=f"Scholion deletion plan: {plan.document_id}")
...
def _render_retention_plan(plan: RetentionPlan) -> None:
    table = Table(title="Scholion private execution-state retention plan")
...
def _handle_error(exc: Exception) -> None:
    ...
    typer.echo(
        f"Scholion custody operation failed internally ({type(exc).__name__})",
        err=True,
    )
