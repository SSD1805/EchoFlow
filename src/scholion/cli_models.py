...
from scholion.model_management.models import ManagedModelManifest, ModelInventoryItem
...
def _render_inventory(items: tuple[ModelInventoryItem, ...], console: Console) -> None:
    table = Table(title="Scholion managed models")
    ...
    try:
        ...
        if not manager.is_installed(model_id):
            raise typer.BadParameter(
                "model is not managed by Scholion", param_hint="MODEL"
            )
        if not yes and not typer.confirm(
            f"Remove Scholion's managed {model_id} model revision from local storage?"
        ):
            raise typer.Abort()
...
def _recommend_model(
    ...
    *,
    ...
    if json_output:
        ...
        return
    table = Table(title="Scholion model recommendation")
    ...
    if not installed:
        table.add_row("Install", f"scholion models install {model_id}")
    Console().print(table)
