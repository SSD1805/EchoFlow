...
from scholion.cli_custody import register_custody_commands
from scholion.cli_saved_searches import register_saved_search_commands
from scholion.core.errors import ScholionError
from scholion.library.research_projector import ResearchProjectionSyncReport
...
def _handle_error(exc: Exception) -> None:
    ...
    typer.echo(
        f"Scholion research workspace failed internally ({type(exc).__name__})",
        err=True,
    )
...
def _render_notes(notes: tuple[ResearchNoteView, ...]) -> None:
    table = Table(title=f"Scholion research notes: {len(notes)}")
