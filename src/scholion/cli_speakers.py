...
def _handle_error(exc: Exception) -> None:
    ...
    typer.echo(
        f"Scholion speaker labels failed internally ({type(exc).__name__})",
        err=True,
    )
...
def _render_roster(
    ...
    table = Table(title=f"Scholion speakers: {document_id}")
...
def _render_transcript(
    ...
    table = Table(title=f"Scholion speaker transcript: {document_id}")
