...
from scholion.core.config import AppConfig, _platform_dirs
from scholion.core.errors import ConfigurationError
from scholion.core.privacy import PathDisclosure
...
def test_local_defaults_are_valid():
    ...
    platform_paths = PlatformDirs("Scholion", appauthor=False)
    ...
    assert platform_paths.user_downloads_path / "Scholion" == config.OUTPUT_DIR
...
def test_environment_overrides_local_settings(monkeypatch, tmp_path):
    monkeypatch.setenv("SCHOLION_APP_ENV", "production")
    monkeypatch.setenv("SCHOLION_DEBUG", "true")
    monkeypatch.setenv("SCHOLION_LOG_LEVEL", "warning")
    monkeypatch.setenv("SCHOLION_LOG_PATHS", "full")
    monkeypatch.setenv("SCHOLION_PROCESSING_PROFILE", "screening")
    monkeypatch.setenv("SCHOLION_MAX_CPU_THREADS", "3")
    monkeypatch.setenv("SCHOLION_MAX_MEMORY_BYTES", "4096")
    monkeypatch.setenv("SCHOLION_MEMORY_BUDGET_FRACTION", "0.5")
    monkeypatch.setenv("SCHOLION_FFPROBE_TIMEOUT_SECONDS", "45")
    monkeypatch.setenv("SCHOLION_FFMPEG_PROCESS_TIMEOUT_SECONDS", "900")
    monkeypatch.setenv("SCHOLION_PYANNOTE_MODEL_ID", "example/local-diarizer")
    monkeypatch.setenv("SCHOLION_PYANNOTE_MODEL_REVISION", "speaker-revision")
    monkeypatch.setenv("SCHOLION_STATE_DIR", str(tmp_path / "state"))
    monkeypatch.setenv("SCHOLION_CACHE_DIR", str(tmp_path / "cache"))
    monkeypatch.setenv("SCHOLION_MODEL_DIR", str(tmp_path / "cache" / "models"))
    monkeypatch.setenv("SCHOLION_OUTPUT_DIR", str(tmp_path / "output"))
    config = AppConfig(_env_file=None)
...
def test_obsolete_asr_revision_environment_override_is_ignored(monkeypatch):
    monkeypatch.setenv("SCHOLION_FASTER_WHISPER_MODEL_REVISION", "unmanaged")
    config = AppConfig(_env_file=None)
...
def test_model_directory_follows_environment_cache_override(monkeypatch, tmp_path):
    monkeypatch.setenv("SCHOLION_CACHE_DIR", str(tmp_path / "environment-cache"))
    config = AppConfig(_env_file=None)
...
def test_local_paths_expand_home(monkeypatch, tmp_path):
    ...
    config = AppConfig(
        ...
        OUTPUT_DIR="~/Downloads/Scholion",
        _env_file=None,
    )
    assert tmp_path / "state" == config.STATE_DIR
    assert tmp_path / "cache/models" == config.MODEL_DIR
    assert tmp_path / "Downloads/Scholion" == config.OUTPUT_DIR


def test_unprefixed_environment_and_ambient_dotenv_are_ignored(monkeypatch, tmp_path):
    monkeypatch.chdir(tmp_path)
    (tmp_path / ".env").write_text("SCHOLION_LOG_LEVEL=CRITICAL\n")
...
def test_empty_namespaced_environment_value_uses_default(monkeypatch):
    monkeypatch.setenv("SCHOLION_LOG_LEVEL", "")
...
def test_explicit_dotenv_is_loaded_but_environment_still_wins(monkeypatch, tmp_path):
    ...
    config_file.write_text(
        "SCHOLION_LOG_LEVEL=WARNING\n"
        "SCHOLION_PROCESSING_PROFILE=screening\n"
        f"SCHOLION_OUTPUT_DIR={tmp_path / 'from-file'}\n"
    )
    monkeypatch.setenv("SCHOLION_LOG_LEVEL", "ERROR")
...
def test_explicit_configuration_file_must_exist(tmp_path):
    ...
    assert str(error.value) == "Scholion configuration file is unavailable: missing.env"
