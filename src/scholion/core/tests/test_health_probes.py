...
from scholion.core.health_check import CheckStatus
...
def test_workspace_probe_reports_write_failure_without_leaking_exception(tmp_path):
    with patch(
        "scholion.core.health_probes.tempfile.NamedTemporaryFile",
...
def test_disk_threshold_boundaries(tmp_path, free, expected):
    ...
    with patch("scholion.core.health_probes.shutil.disk_usage", return_value=usage):
...
def test_disk_probe_walks_to_existing_parent_for_missing_workspace(tmp_path):
    ...
    with patch("scholion.core.health_probes.shutil.disk_usage") as disk_usage:
...
def test_ffmpeg_missing_is_optional_warning():
    with patch("scholion.core.health_probes.shutil.which", return_value=None):
...
def test_ffmpeg_is_invoked_without_a_shell():
    ...
    with (
        patch(
            "scholion.core.health_probes.shutil.which", return_value="/usr/bin/ffmpeg"
        ),
        patch(
            "scholion.core.health_probes.subprocess.run", return_value=completed
        ) as run,
    ):
...
def test_ffmpeg_timeout_is_warning():
    with (
        patch("scholion.core.health_probes.shutil.which", return_value="ffmpeg"),
        patch(
            "scholion.core.health_probes.subprocess.run",
...
def test_ffmpeg_nonzero_exit_is_warning():
    ...
    with (
        patch("scholion.core.health_probes.shutil.which", return_value="ffmpeg"),
        patch("scholion.core.health_probes.subprocess.run", return_value=completed),
    ):
        result = FfmpegProbe(0.1).check()
