...
from scholion.library.research_state import ResearchNote, ResearchStateStore
from scholion.library.semantic import SemanticIndex
from scholion.library.service import (
    LibraryEvidenceReceipt,
    SourceIntegrity,
    TranscriptLibraryService,
)
from scholion.library.workspace_metadata import WorkspaceMetadataStore
...
class LibraryCustodyService:
    ...
    def _validate_workspace_path(self, workspace: Path) -> None:
        ...
        if resolved.parent != jobs_dir:
            raise CustodyOperationError(
                "Refusing to delete a workspace outside Scholion's private "
                "jobs directory"
            )
