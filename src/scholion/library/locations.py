...
Scholion permission to discover files there at explicit lifecycle points; it never grants
...
from scholion.library.errors import LibraryLocationError
from scholion.library.service import LibraryRefreshReport, TranscriptLibraryService
...
class JsonLibraryLocationStore:
        ...
        try:
            ...
            if state.schema_version != _STATE_SCHEMA_VERSION:
                raise LibraryLocationError(
                    "Library location state was written by an unsupported Scholion schema"
                )
            locations = tuple(
                LibraryLocation(
    ...
    def replace_all(self, locations: tuple[LibraryLocation, ...]) -> None:
        if len(locations) > _MAX_LOCATIONS:
            raise LibraryLocationError(
                f"Scholion cannot remember more than {_MAX_LOCATIONS} library locations"
            )
        self._validate_unique(locations)
...
class LibraryLocationService:
    ...
    def _validate_new_root(self, path: str | Path) -> Path:
        ...
        if any(resolved == root or root in resolved.parents for root in private_roots):
            raise LibraryLocationError(
                "Private Scholion application state cannot be registered as a library location"
            )
        if resolved == self.paths.output_dir.resolve(strict=False):
            raise LibraryLocationError(
                "Scholion's configured output directory is already discovered automatically"
            )
        return resolved
