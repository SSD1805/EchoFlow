...
def run_module(*arguments: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "scholion", *arguments],
        capture_output=True,
        check=False,
        text=True,
    )
...
def test_real_init_json_keeps_logs_on_standard_error(tmp_path) -> None:
    environment = {
        **os.environ,
        "SCHOLION_STATE_DIR": str(tmp_path / "state"),
        "SCHOLION_CACHE_DIR": str(tmp_path / "cache"),
        "SCHOLION_OUTPUT_DIR": str(tmp_path / "output"),
    }
    result = subprocess.run(
        [sys.executable, "-m", "scholion", "init", "--json"],
        capture_output=True,
        check=False,
        text=True,
        env=environment,
...
def test_real_dry_run_probes_media_then_refuses_unmanaged_model_without_writes(
    ...
    environment = {
        **os.environ,
        "SCHOLION_STATE_DIR": str(state),
        "SCHOLION_CACHE_DIR": str(cache),
        "SCHOLION_OUTPUT_DIR": str(output),
        "SCHOLION_FFPROBE_TIMEOUT_SECONDS": "10",
    }
    result = subprocess.run(
        [
            sys.executable,
    ...
    assert "scholion models install tiny" in result.stderr
