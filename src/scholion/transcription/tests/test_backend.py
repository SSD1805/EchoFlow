...
from scholion.transcription.alignment import aligned_words
...
def test_missing_optional_engine_dependency_has_install_instruction(tmp_path, failure):
    ...
    with pytest.raises(
        TranscriptionDependencyError,
        match="^CPU transcription support is not installed; install Scholion's transcription extra$",
    ):
...
def test_model_initialization_failure_is_local_managed_model_error(tmp_path):
    ...
    with pytest.raises(
        ModelUnavailableError,
        match="^Managed model 'tiny' is unavailable locally; run `scholion models install tiny` to reinstall it$",
    ) as error:
