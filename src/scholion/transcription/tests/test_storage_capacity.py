...
from scholion.transcription.storage import (
...
def test_same_filesystem_allocations_are_summed_before_admission(
    tmp_path: Path,
) -> None:
...
def test_inspector_uses_existing_ancestor_for_future_destination(
    tmp_path: Path,
) -> None:
...
def test_inspector_failure_is_typed_without_disclosing_path(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "scholion.transcription.storage.shutil.disk_usage", side_effect=OSError()
        ),
        pytest.raises(
            ResourceAdmissionError,
